...
每一份笔记都是一个自包含的 HTML 思维导图（样式、字体全部内嵌），支持**点击节点折叠 / 展开**，并带公式排版（KaTeX）。
...
- **点击节点左侧的小圆点** → 折叠 / 展开该节点的所有子节点；
- 页面**右上角工具栏** → `全部展开` / `全部折叠` 一键切换（折叠适合先看全局骨架，再逐层展开细节）；
- 公式由 KaTeX 渲染，离线打开时公式字体走 CDN，联网即可正常显示。
...
- 笔记内容为个人学习整理，思维导图由工具导出为零依赖的静态 HTML。
- 折叠 / 展开交互是一段注入的轻量脚本，位于每个 HTML 文件末尾的 `injected: interactive collapse` 注释之间，删除它即可还原为原始静态导图。
